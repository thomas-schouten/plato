...
class PlateTorques():
    ...
    def remove_net_rotation(
        ...
        for _age in _tqdm(
            ...
            for key, entries in _iterable.items():
                ...
                # Filter plates by plateID
                if plateIDs is not None:
                    _plate_data = _plate_data[_plate_data.plateID.isin(_plateIDs)]

                # Filter plates by area
                if self.settings.options[key]["Minimum plate area"] > 0:
                    _plate_data = _plate_data[_plate_data.area >= self.settings.options[key]["Minimum plate area"]]
        ...
        self.calculate_net_rotation(ages, cases, plateIDs, PROGRESS_BAR=False, VERSION=VERSION)
    ...
    def sample_all(
        ...
        # Sample LAB depths
        self.calculate_lab_depths(ages, cases, plateIDs, PROGRESS_BAR)
    ...
    def calculate_synthetic_velocity(
        ...
        # Calculate the synthetic velocity at points
